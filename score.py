...
_EXCUSE = (
    ...
    # ⚠ 2026-08-16 에 두 번 새어 나간 꼴. 위 목록은 '도구가 없' 처럼 조사가
    #   붙은 형태만 잡아서, "제 도구로는 ~ 방법이 없어요" 가 그대로 통과했다.
    #   그날 실제로 나간 말:
    #     "날씨 조회 도구가 저한테는 없어서, 확인해서 알려드릴 방법이 없어요"
    #     "지금 제 쪽 도구로는 혼자 깨어나서 30분마다…"
    #   둘 다 실제로는 **되는 일** 이었다 — 능동 루프가 이미 돌고 있었다.
    "도구로는", "도구가 저한테", "도구는 없", "방법이 없어",
    "혼자 깨어날 수 없", "제 쪽에서는 안",
)
