...
# 받아쓰기는 '목소리' 를 자주 흘린다 — 실측 '목소를', '목소', '목쏘리'.
# 한 글자가 빠지면 등록 판정이 통째로 빗나가고, 그대로 클로드로 샌다.
# 클로드에는 마이크가 없어서 "로컬에서 처리해야 합니다" 하고 멈춘다 —
# 사장님이 "왜 자꾸 클로드와 로컬이 왔다 갔다" 하신 게 이것이다 (2026-08-17).
# 그래서 낱말을 통째로 요구하지 않고 '목소' 까지만 맞아도 받는다.
_VOICE_LEARN_OBJ = ("목소", "목쏘", "음성", "화자")
...
# '지금 이 목소리 한 번 배워' 와 '제대로 등록해' 는 다른 부탁이다.
#
# ⚠ 2026-08-17 사고. 사장님께 "동백아, 내 목소리 등록해줘 하시면 5문장을
#   불러줍니다" 라고 안내했는데, 실제로는 "지금 목소리를 홍길동 목소리로
#   기억했습니다" 한마디로 끝났다. 위 is_voice_enroll_request 가 '목소리 +
#   등록' 을 통째로 삼키고 continue 해서, 정식 5문장 등록(enroll_by_voice)
#   에는 음성으로 **도달할 방법이 없었다.** 죽은 코드였다.
#
#   경계를 말뜻대로 긋는다.
#     "기억해/외워/저장해"  → 지금 이 한 마디를 지문에 보탠다 (가볍다)
#     "등록해/추가해"        → 5문장을 받아 제대로 만든다 (무겁다)
#
#   단, 방금 목소리를 못 알아봐 거절한 발화가 있으면 그쪽이 먼저다.
#   그때 사장님이 원하시는 건 절차가 아니라 복구다 — 다섯 문장을 읽으라고
#   하면 그게 더 답답하다. 그 판단은 부르는 쪽(dongbaek)에서 한다.
_ENROLL_FORMAL = re.compile(r"(목[소쏘]리?|음성|화자)\s*(를|을|도|리|린)?\s*(등록|추가)")


# 이미 해놓은 것을 묻는 꼴. "등록해놨어?" 는 부탁이 아니라 질문이다.
# 여기 안 걸러내면 물어보실 때마다 다섯 문장을 읽으라고 한다 —
# 2026-08-13 에 "기억하고 있는 거지?" 로 똑같은 사고가 이미 있었다.
_ENROLL_DONE = re.compile(r"(등록|추가)(해)?(놨|뒀|났|했|돼|됐|되어)")


def wants_formal_enroll(text: str) -> bool:
    """5문장 정식 등록을 원하는 말인가 ('기억해' 류나 질문은 아니다)."""
    s = normalize(text).replace(" ", "")
    if not _ENROLL_FORMAL.search(s):
        return False
    # 상태를 묻는 꼴은 등록 요청이 아니다 — 위와 같은 이유.
    if any(k in s for k in _VOICE_LEARN_STATE):
        return False
    if _ENROLL_DONE.search(s):
        return False
    return not s.endswith(_VOICE_LEARN_Q_END)
...
# "지금 티비 보고 있어" — 통화 선언("통화중이야")과 같은 뜻이다. 옆에서 나는
# 소리는 사람 말이 아니라 화면이니 끼어들지 말라는 것.
#
# 2026-08-15 저녁 실측: 사장님이 이 말을 여섯 번 하셨는데 여섯 번 다 클로드로
# 올라갔다($0.59). 그때마다 대화창이 새로 열려 이어진 TV 소리가 또 붙었고,
# 17:27 에는 "티비 다 볼 때까지는 얘기하지 마" 까지 하셔야 했다. 말귀를
# 못 알아들으니 같은 말을 되풀이하시게 되는 자리다.
#
# ⚠ 끝맺는 꼴만 본다. "보고있는데"·"보고있으니까" 는 뒤에 용건이 따라오는
#   말이라 받아야 한다 ("티비 보고 있는데 지금 몇 시야?"). 그래서 '있' 뒤에
#   종결어미가 오는 것만 잡는다 — 연결어미(는·으)는 저절로 빠진다.
# ⚠ ECHO_TAIL 로 용건을 가르는 안은 버렸다. 그 규칙에 '있어' 가 들어 있어
#   "TV 보고 있어" 자체가 지시로 잡힌다 — 잡아야 할 것부터 놓친다.
# ⚠ 길이는 위와 같은 30자를 쓴다. 긴 말은 선언이 아니라 받아쓰기다.
#   들림 로그 7,400건에 통째로 돌려 11건이 걸리고 전부 진짜 선언이었다.
_MEDIA_WATCH = re.compile(
    r"(티비|tv|티브이|유튜브|넷플릭스|드라마|방송|영화|뉴스|라디오)[을를]?"
    r"(?:(?:보고|듣고|틀어놓고|켜놓고)있[어다음네요]"
    r"|(?:보는|듣는)(?:중|거)(?:이야|이에요|이다|야|예요|다|고|지)"
    r"|시청중이)")
...
# "이 정도 음향 괜찮아?" — 뜻은 위와 같은데 말이 다르다. 2026-08-17 09:07 에
# 이 말이 클로드로 올라가 "음향 상태 확인할 수 있는 도구가 없어서" 라고
# 답했다. 열네 분 전에 저 스스로 입력 볼륨을 30에서 65로 올려놓고 한 말이고,
# 데몬은 발화마다 증폭 배수를 재고 있다 — 도구가 없는 게 아니라 안 본 것이다.
# 없는 이유를 지어내는 건 채점에서 제일 무거운 잘못이다 (CLAUDE.md).
_SOUND_ASK = ("음향괜찮", "음향어때", "음향은어때", "음향이어때",
              "소리어때", "소리는어때", "소리괜찮",
              "마이크어때", "마이크괜찮", "마이크잘",
              "음질어때", "음질괜찮")

# ⚠ '그것만 물었을 때' 만 받는다. 뒤에 딴 부탁이 붙어 있으면 한 줄짜리 상태
#   답이 그 부탁을 통째로 삼킨다 — 같은 날 아침 메일 즉답이 '확인' 두 글자만
#   보고 말을 가로챈 것과 같은 사고다. 상한은 시각 질문(_is_time_query)과 같은
#   12자다. ("음향괜찮아그리고메일확인해줘" 가 14자라 이 선에서 갈린다)
_SOUND_ASK_MAX_LEN = 12


def is_far_status_query(t: str) -> bool:
    s = t.replace(" ", "")
    if any(k in s for k in _FAR_ASK):
        return True
    return len(s) <= _SOUND_ASK_MAX_LEN and any(k in s for k in _SOUND_ASK)
...
_ENROLL = re.compile(r"(목[소쏘]리?|음성|화자)\s*(를|을|도|리|린)?\s*(등록|추가|기억)")
...
def is_wake_like(name: str) -> bool:
    """호출어(동백아·동백이…)를 이름으로 삼으려는 것인가.

    ⚠ 2026-08-17 사고. 지문에 '동백아' 5개, '동백이' 1개가 생겼다.
      사장님이 "동백아, 내 목소리 등록해줘" 하셨는데 호출어가 이름 자리로
      들어갔고, 이름을 되물었을 때 하신 대답도 '동백이' 로 받아써졌다.
      결과는 사장님 목소리가 홍길동·동백아·동백이 셋으로 쪼개진 것이다.

      지문이 쪼개지면 어느 쪽으로도 문턱을 못 넘어 "목소리를 못 알아봐
      무시했다" 가 늘어난다. 고치려고 등록했는데 더 나빠진다.

      호출어는 동백 자신을 부르는 말이라 사람 이름일 수가 없다.
    """
    s = (name or "").strip().replace(" ", "")
    if not s:
        return False
    wake = {w.replace(" ", "") for w in getattr(config, "WAKE_WORDS", [])}
    if s in wake:
        return True
    # '동백' 을 품은 꼴은 전부 막는다 — 받아쓰기가 '동백이가', '동백아까'
    # 처럼 꼬리를 붙여 적어도 사람 이름은 아니다.
    return "동백" in s or "똥백" in s
...
def enroll_request(text: str) -> str | None:
    ...
    if not _ENROLL.search(normalize(text)):
        return None
    # 두 자리 모두 호출어를 막는다. "동백아 내 목소리 등록해줘" 의
    # '동백아' 가 이름 자리로 들어가 지문이 그 이름으로 생긴 적이 있다.
    for pat in (_ENROLL_NAME_TAG, _ENROLL_NAME_PRE):
        m = pat.search(text)
        if not m:
            continue
        n = _trim_name(m.group(1))
        if n and n not in _NOT_A_NAME and not is_wake_like(n):
            return n
    return ""
...
def clean_name(text: str) -> str:
    ...
    # ⚠ '이름은 …' 꼴로 걸려도 호출어면 버리고 계속 훑는다. 여기서 바로
    #   돌려주다 '이름은 동백이야' 가 '동백' 으로 새어 나갔다 (2026-08-17).
    ...
    for w in t.split():
        name = _trim_name(w)
        # 호출어는 사람 이름이 아니다 — 되물었을 때 "동백이" 로 받아써져
        # 지문이 그 이름으로 생긴 적이 있다 (2026-08-17).
        if name and name not in _NOT_A_NAME and not is_wake_like(name):
            return name
    return ""
...
# 로컬 즉답이 답할 수 있는 메일 질문은 '뭐 왔나' 하나뿐이다 — 통수(unread_count)
# 아니면 최신 3건의 보낸이·제목(recent). 본문을 읽고 판단해 달라거나 파일을
# 받아 달라는 말에는 애초에 답이 없으니, 여기서 물러나 클로드(메일 검색·읽기·
# 첨부 도구)에게 넘긴다.
#
# 2026-08-17 09:16~09:20 실측 4건. "어제 더비안 강남에서 온 메일이 있어. 이거
# 확인해 가지고 니즈 파악하고 내용 분석 좀 해줘" 가 '확인' 두 글자에 걸려
# 최신 3건 목록으로 답했고, 사장님이 말을 바꿔 세 번 더 이르시는 동안 네 번
# 다 똑같은 목록이 나갔다 ("왜 얘기해 말을 하다 말어"). 09:42 에 '확인' 을
# 빼고 "첨부 받아줘" 라고 하셔서야 클로드로 올라가 7개를 제대로 받았다.
# 0.4초짜리 즉답이 정작 하실 일을 30분 가로막은 자리다.
#
# ⚠ 로컬을 좁히는 쪽이라 잘못 걸려도 클로드가 받는다 — 즉답이 늦어질 뿐
#   답이 사라지지는 않는다. 반대 방향(넓히기)이었으면 이렇게 못 짠다.
_MAIL_DEEP = ("요약", "정리", "취합", "분석", "파악", "보고",
              "내용", "본문", "첨부", "다운", "파일")

# 보낸 이를 짚어 찾으시는 말 — "한빛건설 강남에서 온 메일". 위 4건 중 09:17 의
# "더비안 강남에서 보낸 메일을 확인하라고" 는 읽어 달라는 낱말이 하나도 없어
# 낱말 목록만으로는 안 걸린다. 짚으신 이름이 무엇이든 '최신 3건' 은 그 말의
# 답이 아니므로, 이름이 앞에 서면 그것만으로 물러난다.
#
# ⚠ '오늘·최근' 같은 때를 따로 빼지 않았다. 지난 대화 2,551건에 돌려 걸린
#   것은 7건뿐이고 전부 진짜 발신인이었다 — 때를 짚든 회사를 짚든 목록으로는
#   답이 안 되는 건 마찬가지라, 빼는 쪽이 오히려 구멍이 된다.
_MAIL_FROM = re.compile(
    r".{2,12}(?:에서|에게서|한테서|으로부터|로부터|께서)(?:온|보낸|오는|받은)?메일")


def _is_mail_deep(t: str) -> bool:
    """본문·파일까지 봐야 답이 되는 말인가. 그러면 로컬 즉답이 물러난다."""
    return any(k in t for k in _MAIL_DEEP) or bool(_MAIL_FROM.search(t))


def _is_mail_unread(t: str) -> bool:
    if "메일" not in t or _is_mail_deep(t):
        return False
...
# '회의 모드', '전화 모드' 는 동백의 **기능 이름**이지 회의가 아니다.
#
# ⚠ 2026-08-17. 사장님이 "그럼 그거 개발해 줘. 지금 회의 모드, 전화 모드,
#   지피티 모드를 고도화 시켜줘" 하셨는데 동백이 "개발해 관련 일정은 찾지
#   못했어요" 라고 답했다. '회의' 를 일정 낱말로 읽고 남은 '개발해' 를
#   검색어로 삼은 것이다. 개발 부탁이 캘린더 조회로 새 버렸다.
#
#   '전화 모드 고도화해줘' 는 안 걸리고 '회의 모드 개발해 줘' 만 걸렸다 —
#   일관성도 없었다. 걸린 쪽만 고치면 다음엔 '미팅 모드' 로 또 난다.
#   그래서 낱말이 아니라 **꼴**로 막는다: 일정 낱말 뒤에 '모드' 가 붙으면
#   그건 기능 이름이라 일정 판정에서 아예 빼고 본다.
#
#   지우지 않고 빼고 보는 이유 — "회의 모드 고치고, 내일 회의 언제야" 처럼
#   한 문장에 둘 다 있을 수 있다. 뒤쪽 진짜 회의는 그대로 살아야 한다.
_MODE_NAME = re.compile(
    r"(회의|미팅|통화|전화|화상|지피티|GPT|gpt|개발|운전|수면)\s*모드")


def _is_schedule_query(text: str) -> bool:
    """'조회'인 일정 질문만 참. 쓰기 동사가 섞이면 거짓(→ 위험 게이트로)."""
    t = _MODE_NAME.sub(" ", normalize(text))
    if not any(w in t for w in _SCHEDULE_WORDS):
        return False
...
_VERBISH = re.compile(
    ...
    # 개발을 부탁하는 말도 일정 이름이 아니다. "회의 모드 고치고, 내일
    # 회의 언제야" 에서 '고치고' 가 검색어가 됐다 (2026-08-17).
    r"고치|고쳐|손보|손봐|개선|개발|고도화|구현|만들어|바꿔|수정|붙여|"
